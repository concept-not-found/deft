start = Whitespace Expression? Whitespace
...
Identifier = !ReservedWord name:IdentifierName {
  return name
}

IdentifierName "identifier" = head:IdentifierStart tail:IdentifierPart* {
  return {
    type: 'Identifier',
    name: `${head}${tail.join('')}`
  }
...
ReservedWord = 'typeof' !IdentifierPart
  / Null
  / Boolean
...
Numeric = literal:HexIntegerLiteral !(IdentifierStart / DecimalDigit) {
  return literal
}
...
HexIntegerLiteral = "0x"i digits:$HexDigit+ {
  return {
    type: 'Numeric',
...
DoubleStringCharacter = !('"' / "\\" / LineTerminator) SourceCharacter { return text(); }
  / "\\" sequence:EscapeSequence { return sequence; }

SingleStringCharacter = !("'" / "\\" / LineTerminator) SourceCharacter { return text(); }
  / "\\" sequence:EscapeSequence { return sequence; }
...
NonEscapeCharacter = !(EscapeCharacter / LineTerminator) SourceCharacter { return text(); }
...
HexEscapeSequence = "x" digits:$(HexDigit HexDigit) {
  return String.fromCharCode(parseInt(digits, 16));
}

UnicodeEscapeSequence = "u" digits:$(HexDigit HexDigit HexDigit HexDigit) {
  return String.fromCharCode(parseInt(digits, 16));
}
